...
FreeDagNode::copyEagerUptoReduced2(const bool flag) //MAU-DEV
{
  FreeSymbol* s = symbol();
  ...
  if (nrArgs != 0)
      ...
      if (s->standardStrategy())  // everything is eager
	{
	  for (int i = nrArgs; i > 0; i--, p++, q++)
        ...
        *q = (*p)->copyEagerUptoReduced(flag); //MAU-DEV
	}
      else
	{
	  for (int i = 0; i < nrArgs; i++, p++, q++)
	    //*q = s->eagerArgument(i) ? (*p)->copyEagerUptoReduced() : *p;
        *q = s->eagerArgument(i) ? (*p)->copyEagerUptoReduced(flag) : *p; //MAU-DEV
	}
    }
  if (isHole()) //MAU-DEV
      n->setHole(); //MAU-DEV
