	...
	if (a1->symbol() == quotedIdentifierSymbol)
	  {
        ...
        {
            DagNode* lhs = subject->copyReducible(true);
	...
	if (a1->symbol() == stringSymbol)
	  {
	    ...
	    if (nrIds == 0)
	      return context.builtInReplaceRecord(subject, nilQidListSymbol->makeDagNode()); //MAU-DEV

	    PointerMap qidMap;
	    if (nrIds == 1)
	      return context.builtInReplaceRecord(subject, makeQid((*ids)[0], qidMap)); //MAU-DEV

	    Vector<DagNode*> args(nrIds);
	    for (int i = 0; i < nrIds; i++)
	      args[i] = makeQid((*ids)[i], qidMap);
	    DagNode* result = qidListSymbol->makeDagNode(args);
	    return context.builtInReplaceRecord(subject, result); //MAU-DEV
	  }
	break;
      }
#ifdef MOS
