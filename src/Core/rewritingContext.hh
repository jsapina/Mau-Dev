...
public:
  ...
  void saveEquation(int eqIndex);
  void saveMembership(int scIndex);
  void addBuiltIn(DagNode* lhs, DagNode* subject);
  void addMemoized(DagNode* lhs, DagNode* subject);

  void startRecording(bool membership);
  void stopRecording();
  bool isRecording();
  bool isRecordingMembership();
  int getEquation(); //equation index within module (-1 if builtIn or membership)
  int getMembership(); //membership index within module
  /*** END MAU-DEV ***/
...
/*** BEGIN MAU-DEV ***/
inline const Vector<RewritingContext*>& RewritingContext::getContexts() const
{
...
{
   if (!isRecording())
     return builtInReplace(old,replacement);
   DagNode* lhs = old->copyReducible(true);
   bool trace = traceFlag;
   if (trace)
     {
       tracePreEqRewrite(old, 0, BUILTIN);
