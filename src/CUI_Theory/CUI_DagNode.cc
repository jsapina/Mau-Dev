...
CUI_DagNode::copyEagerUptoReduced2(const bool flag) //MAU-DEV
{
  CUI_Symbol* s = symbol();
  ...
  n->argArray[0] = s->eagerArgument(0) ? argArray[0]->copyEagerUptoReduced(flag) : argArray[0]; //MAU-DEV
  n->argArray[1] = s->eagerArgument(1) ? argArray[1]->copyEagerUptoReduced(flag) : argArray[1]; //MAU-DEV
  if (isHole()) //MAU-DEV
      n->setHole(); //MAU-DEV
  return n;
}
