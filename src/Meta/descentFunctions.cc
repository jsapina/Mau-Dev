...
/*** BEGIN MAU-DEV ***/
bool MetaLevelOpSymbol::metaAssociative(FreeDagNode* subject, RewritingContext& context)
{
	if (ImportModule* m = metaLevel->downModule(subject->getArgument(0))) {
		if (Term* t = metaLevel->downTerm(subject->getArgument(1), m)) {
			DagNode* d =  metaLevel->upBool(m->getSymbolType(t->symbol()).hasFlag(SymbolType::ASSOC));
			t->deepSelfDestruct();
			return context.builtInReplace(subject,d);
		}
	}
...
bool MetaLevelOpSymbol::metaCommutative(FreeDagNode* subject, RewritingContext& context)
{
	if (ImportModule* m = metaLevel->downModule(subject->getArgument(0))) {
		if (Term* t = metaLevel->downTerm(subject->getArgument(1), m)) {
			DagNode* d =  metaLevel->upBool(m->getSymbolType(t->symbol()).hasFlag(SymbolType::COMM));
			t->deepSelfDestruct();
			return context.builtInReplace(subject,d);
		}
	}
...
bool MetaLevelOpSymbol::metaConstructor(FreeDagNode* subject, RewritingContext& context)
{
	if (ImportModule* m = metaLevel->downModule(subject->getArgument(0))) {
		if (Term* t = metaLevel->downTerm(subject->getArgument(1), m)) {
            DagNode* dt = term2Dag(t);
			DagNode* d =  metaLevel->upBool(dt->symbol()->isConstructor(dt));
			t->deepSelfDestruct();
			return context.builtInReplace(subject,d);
		}
	}
...
bool MetaLevelOpSymbol::metaIdentity(FreeDagNode* subject, RewritingContext& context)
{
	if (ImportModule* m = metaLevel->downModule(subject->getArgument(0))) {
		if (Term* t = metaLevel->downTerm(subject->getArgument(1), m)) {
			...
			if (st.hasFlag(SymbolType::LEFT_ID) && st.hasFlag(SymbolType::RIGHT_ID))
				d = metaLevel->upIdentity(m, symbol, qidMap);
			else
				d = metaLevel->upNoIdentity();
			t->deepSelfDestruct();
			return context.builtInReplace(subject,d);
		}
	}
...
bool MetaLevelOpSymbol::metaRightIdentity(FreeDagNode* subject, RewritingContext& context)
{
	if (ImportModule* m = metaLevel->downModule(subject->getArgument(0))) {
		if (Term* t = metaLevel->downTerm(subject->getArgument(1), m)) {
			Symbol* symbol = t->symbol();
			...
			PointerMap qidMap;
			DagNode* d;
			if (st.hasFlag(SymbolType::RIGHT_ID))
				d = metaLevel->upIdentity(m, symbol, qidMap);
			else
				d = metaLevel->upNoIdentity();
			t->deepSelfDestruct();
...
bool MetaLevelOpSymbol::metaLeftIdentity(FreeDagNode* subject, RewritingContext& context)
{
	if (ImportModule* m = metaLevel->downModule(subject->getArgument(0))) {
		if (Term* t = metaLevel->downTerm(subject->getArgument(1), m)) {
			Symbol* symbol = t->symbol();
...
bool MetaLevelOpSymbol::metaOutermost(FreeDagNode* subject, RewritingContext& context)
{
	if (MixfixModule* m = metaLevel->downModule(subject->getArgument(0))){
		if (Term* t = metaLevel->downTerm(subject->getArgument(1), m)) {
			PointerMap qidMap;
			DagNode* d = metaLevel->upOutermost(term2Dag(t), m, qidMap);
			t->deepSelfDestruct();
			return context.builtInReplace(subject,d);
		}
	}
...
bool MetaLevelOpSymbol::metaString(FreeDagNode* subject, RewritingContext& context)
{
	if (MixfixModule* m = metaLevel->downModule(subject->getArgument(0))) {
		if (Term* t = metaLevel->downTerm(subject->getArgument(1), m)) {
			bool source;
			metaLevel->downBool(subject->getArgument(2),source);
			stringstream buffer;
			if (source)
				buffer << t;
			else {
				PointerMap qidMap;
				PointerMap dagNodeMap;
				DagNode* d = metaLevel->upDagNode(term2Dag(t), m, qidMap, dagNodeMap);
				buffer << d;
			}
...
bool MetaLevelOpSymbol::metaMap(FreeDagNode* subject, RewritingContext& context)
{
	if (MixfixModule* m = metaLevel->downModule(subject->getArgument(0))) {
		if (Term* t = metaLevel->downTerm(subject->getArgument(1), m)) {
			stringstream buffer;
			Vector<int> position;
			position.clear();
			m->mapPrettyPrint(position, buffer, t, UNBOUNDED, UNBOUNDED, 0, UNBOUNDED, 0, false);
			string str = buffer.str();
			const char* text = str.c_str();
			t->deepSelfDestruct();
			return context.builtInReplace(subject, metaLevel->upString(text,m));
...
bool MetaLevelOpSymbol::metaReducePath(FreeDagNode* subject, RewritingContext& context)
{
  if (ImportModule* m = metaLevel->downModule(subject->getArgument(0)))
    {
      if (Term* t = metaLevel->downTerm(subject->getArgument(1), m))
	{
      bool membership;
      metaLevel->downBool(subject->getArgument(2),membership);
      DagNode* d = term2Dag(t);
	  t->deepSelfDestruct();
	  RewritingContext* objectContext = context.makeSubcontext(d, UserLevelRewritingContext::META_EVAL);
	  objectContext->startRecording(membership);
      m->protect();
	  objectContext->reduce();
	  context.addInCount(*objectContext);
      d = metaLevel->upITrace(objectContext->root(), m, objectContext->getContexts());
	  objectContext->stopRecording();
	  delete objectContext;
	  (void) m->unprotect();
	  return context.builtInReplace(subject, d);
	}
