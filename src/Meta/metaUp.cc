...
void MetaLevel::getOutermost(DagNode* dagNode, PointerSet& outers)
{
	if (!dagNode->symbol()->isConstructor(dagNode)){
		if (!(outers.contains(dagNode)))
			outers.insert(dagNode);
	}
	else {
		DagArgumentIterator a(*dagNode);
		for(; a.valid(); a.next())
			getOutermost(a.argument(), outers);
	}
}

DagNode* MetaLevel::upOutermost(DagNode* dagNode, MixfixModule* m, PointerMap& qidMap)
{
	PointerSet outers;
	getOutermost(dagNode,outers);

	int nrTerms = outers.cardinality();
	if (nrTerms == 0)
		return emptyTermListSymbol->makeDagNode();

	PointerMap dagNodeMap;
	if (nrTerms == 1){
		DagNode* d = static_cast<DagNode*>(outers.index2Pointer(0));
		return upDagNode(d, m, qidMap, dagNodeMap);
	}

	Vector<DagNode*> args;
	args.clear();
	for(int i = 0; i < nrTerms; i++){
		DagNode* d = static_cast<DagNode*>(outers.index2Pointer(i));
		args.append(upDagNode(d, m, qidMap, dagNodeMap));
	}
	return metaArgSymbol->makeDagNode(args);
}

DagNode* MetaLevel::upNoIdentity()
{
  return noIdentitySymbol->makeDagNode();
}

int MetaLevel::countHoles(DagNode* dagNode)
{
  Assert(dagNode != 0, "null dagNode");
  int res = 0;
  if (dagNode->isHole())
    return 1;
  int nrArgs = dagNode->symbol()->arity();
  if (nrArgs == 0)
    return 0;
  DagArgumentIterator a(*dagNode);
  if (nrArgs == 1)
    return countHoles(a.argument());
  else
    for (; a.valid(); a.next())
      res += countHoles(a.argument());
  return res;
}

DagNode* MetaLevel::upIdentity(MixfixModule* m, Symbol* symbol, PointerMap& qidMap)
{
  Term* identity = safeCast(BinarySymbol*, symbol)->getIdentity();
  return upTerm(identity, m, qidMap);
}

DagNode* MetaLevel::upITrace(DagNode* dagNode, MixfixModule* m, Vector<RewritingContext*> contexts)
{
  Assert(dagNode != 0, "null dagNode");
  int nrSteps = contexts.length();
  if (nrSteps == 0)
    return nilITraceSymbol->makeDagNode();
  ...
  Equation* equation = 0;
  SortConstraint* sc;
  int nholes, eqIndex, vIndex;
  Vector<ConditionFragment*> noCondition;
  vIndex = 0;
  ...
  for (int i = 0; i < nrSteps; i++)
  {
    ...
    {
		sc = m->getSortConstraints()[contexts[i]->getMembership()];
	}
	else
		equation = m->getEquations()[eqIndex];
    nholes = countHoles(contexts[i]->root())-1;
    if (nholes > 0)
...
{
  Assert(equation != 0, "null equation");
  static Vector<DagNode*> args(3);
...
pair<DagNode*, int> MetaLevel::upSuperContext(DagNode* dagNode, MixfixModule* m, PointerMap& qidMap, PointerMap& dagNodeMap, DagNode* redex, int idx)
{
  Assert(dagNode != 0, "null dagNode");
  //
  //	See if our node is the hole.
  ...
  if (dagNode->isHole())
  {
    idx--;
    if (idx == 0)
        return make_pair(holeSymbol->makeDagNode(),idx);
    return make_pair(upDagNode((idx>0?dagNode:redex), m, qidMap, dagNodeMap),idx);
  }
  //
  //	Constants cannot contain the hole.
  //
  ...
  int nrArgs = s->arity();
  if (nrArgs == 0)
    return make_pair(upDagNode(dagNode, m, qidMap, dagNodeMap),idx);
  //
  //	Usual case.
  //
  Vector<DagNode*> args(2);  // can't be static!
  ...
  if (nrArgs == 1)
  {
    pair<DagNode*,int> res = upSuperContext(a.argument(), m, qidMap, dagNodeMap,redex,idx);
    args[1] = res.first;
    idx = res.second;
  }
  else
  ...
  DagNode* d = metaTermSymbol->makeDagNode(args);
  (void) dagNodeMap.setMap(dagNode, d);
  return make_pair(d,idx);
}

DagNode*
MetaLevel::upVariantExt(
             const Vector<DagNode*>& variantExtOld,
             const Vector<DagNode*>& variantExtSub,
             int variantEq,
             Int64 solutionNr,
             ...
             MixfixModule* m)
{
  ...
  Equation* equation = m->getEquations()[variantEq];
  int nrVariables = variant.size() - 1;
  int nrEqVariables = equation->getNrRealVariables();
  
  args[0] = upDagNode(variant[nrVariables], m, qidMap, dagNodeMap);
  args[1] = upSubstitution(variant, variableInfo, nrVariables, m, qidMap, dagNodeMap);
  args[2] = upQid(variableFamilyName, qidMap);
  args[3] = (parentIndex >= 0) ? succSymbol->makeNatDag(parentIndex) : noParentSymbol->makeDagNode();
  args[4] = upBool(moreInLayer);
  if (solutionNr > 0)
  {
    args[5] = upSuperContext(variantExtSub[nrEqVariables], m, qidMap, dagNodeMap,0,1).first;
    args[6] = upEq(equation, m, qidMap);
    args[7] = upSubstitution(variantExtSub, *equation, nrEqVariables, m, qidMap, dagNodeMap); 
    args[8] = upSubstitution(variantExtOld, m, qidMap, dagNodeMap); 
  }
  else
  {
    args[5] = holeSymbol->makeDagNode();
    args[6] = emptyEquationSetSymbol->makeDagNode();
    args[7] = emptySubstitutionSymbol->makeDagNode();
    args[8] = emptySubstitutionSymbol->makeDagNode();
  }
  return variantSymbolExt->makeDagNode(args);
}

DagNode*
MetaLevel::upSubstitution(
              const Vector<DagNode*>& substitution,
              const VariableInfo& variableInfo,
              int nrVariables,
              MixfixModule* m,
              PointerMap& qidMap,
              ...
              PointerMap& dagNodeMap)
{
  ...
  if (nrVariables == 2)
    return upAssignment(
          ...
          qidMap,
          dagNodeMap);
  Vector<DagNode*> args(nrVariables/2);
  for (int i = 0; i < nrVariables; i++)
  {
    args[i/2] = upAssignment(
            substitution[i],
            substitution[i+1],
            m,
            qidMap,
