...
bool
MetaLevelOpSymbol::legacyMetaGetVariant2(FreeDagNode* subject, RewritingContext& context, bool irredundant, bool extended) //MAU-DEV
{
  ...
  if (MetaModule* m = metaLevel->downModule(subject->getArgument(0)))
	  ...
	  metaLevel->downSaturate64(subject->getArgument(4), solutionNr) && solutionNr >= 0)
	{
      ...
      Vector<DagNode*> variantExtOld; //dummy //MAU-DEV
      Vector<DagNode*> variantExtSub; //dummy //MAU-DEV
      int variantEq = 0; //dummy //MAU-DEV
	  ...
	  if (lastSolutionNr == solutionNr)
	    {
	      ...
	      //
	      int dummy;
	      variant = vs->getLastReturnedVariant(variantExtOld, variantExtSub, variantEq, nrFreeVariables, dummy, parentIndex, moreInLayer); //MAU-DEV
	    }
	  else
	    {
		...
		{
		  int dummy;
		  variant = vs->getNextVariant(variantExtOld, variantExtSub, variantEq, nrFreeVariables, dummy, parentIndex, moreInLayer); //MAU-DEV
		  if (variant == 0)
		    {
	    ...
	    }
	  {
        ...
        /*** BEGIN MAU-DEV ***/
        if (extended)
            result = metaLevel->upVariantExt(variantExtOld, variantExtSub, variantEq, solutionNr, *variant, vs->getVariableInfo(), lastVarIndex, parentIndexBig, moreInLayer, m); // dummy args
        else
        /*** END MAU-DEV ***/
...
MetaLevelOpSymbol::legacyMetaGetVariant(FreeDagNode* subject, RewritingContext& context)
{
  ...
  //
  return legacyMetaGetVariant2(subject, context, false, false); //MAU-DEV
}
...
MetaLevelOpSymbol::legacyMetaGetIrredundantVariant(FreeDagNode* subject, RewritingContext& context)
{
  ...
  //
  return legacyMetaGetVariant2(subject, context, true, false); //MAU-DEV
}

/*** BEGIN MAU-DEV ***/
bool
MetaLevelOpSymbol::legacyMetaGetVariantExt(FreeDagNode* subject, RewritingContext& context)
{
  //
  //	op metaGenerateVariant : Module Term TermList Nat Nat ~> Variant? .
  //
  return legacyMetaGetVariant2(subject, context, false, true);
}

bool
MetaLevelOpSymbol::legacyMetaGetIrredundantVariantExt(FreeDagNode* subject, RewritingContext& context)
{
  //
  //	op metaGenerateIrredundantVariant : Module Term TermList Nat Nat ~> Variant? .
  //
  return legacyMetaGetVariant2(subject, context, true, true);
}
/*** END MAU-DEV ***/
