...
bool
MetaLevelOpSymbol::metaGetVariant2(FreeDagNode* subject, RewritingContext& context, bool irredundant, bool extended) //MAU-DEV
{
  ...
  if (MetaModule* m = metaLevel->downModule(subject->getArgument(0)))
	  ...
	  metaLevel->downSaturate64(subject->getArgument(4), solutionNr) && solutionNr >= 0)
	{
	  ...
	  if (lastSolutionNr == solutionNr)
	    {
	      ...
	      //
	      variant = vs->getLastReturnedVariant(variantExtOld, variantExtSub, variantEq, nrFreeVariables, resultVariableFamily, parentIndex, moreInLayer); //MAU-DEV
	    }
	  else
	    {
	      while (lastSolutionNr < solutionNr)
		{
		  variant = vs->getNextVariant(variantExtOld, variantExtSub, variantEq, nrFreeVariables, resultVariableFamily, parentIndex, moreInLayer); //MAU-DEV
		  if (variant == 0)
		    {
	    ...
	    }
	  {
        ...
        /*** BEGIN MAU-DEV ***/
        if (extended)
            result = metaLevel->upVariantExt(variantExtOld, variantExtSub, variantEq, solutionNr, *variant, vs->getVariableInfo(), variableNameId, parentIndexBig, moreInLayer, m);
        else
        /*** END MAU-DEV ***/
...
MetaLevelOpSymbol::metaGetVariant(FreeDagNode* subject, RewritingContext& context)
{
  ...
  //
  return metaGetVariant2(subject, context, false, false); //MAU-DEV
}
...
MetaLevelOpSymbol::metaGetIrredundantVariant(FreeDagNode* subject, RewritingContext& context)
{
  ...
  //
  return metaGetVariant2(subject, context, true, false); //MAU-DEV
}

/*** BEGIN MAU-DEV ***/
bool
MetaLevelOpSymbol::metaGetVariantExt(FreeDagNode* subject, RewritingContext& context)
{
  //
  //	op metaGenerateVariant : Module Term TermList Qid Nat ~> Variant? .
  //
  return metaGetVariant2(subject, context, false, true); //MAU-DEV
}

bool
MetaLevelOpSymbol::metaGetIrredundantVariantExt(FreeDagNode* subject, RewritingContext& context)
{
  //
  //	op metaGenerateIrredundantVariant : Module Term TermList Qid Nat ~> Variant? .
  //
  return metaGetVariant2(subject, context, true, true); //MAU-DEV
}
/*** END MAU-DEV ***/